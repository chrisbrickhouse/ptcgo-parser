...
const specialSetcodes = require('./specialSets')

const SET_PATTERN =
  /(?:\* )?(\d*) ?(.*) ([A-Z]{2,3}|[A-Z]{2}-[A-Z]{2}|[A-Z0-9]{3})? (\d+|XY\d+|BW\d+|TG\d+)/
const BASIC_ENERGY_PATTERN =
  /(?:\* )?(\d+) (Darkness|Fairy|Fighting|Fire|Grass|Lightning|Metal|Psychic|Water) Energy.*/
...
const idFromCard = (card) => {
	...
	// Special case for SWSH promos
	if (promoSet == 'SW') {
	promoSet = 'SWSH'
	}
	}

	if (set && set in specialSetcodes) {
	subSetID = specialSetcodes[set][name]
	}
...
const parseCard = (cardName) => {
	return idFromCard(parseRow(cardName))
}
...
module.exports = { parse, parseCard }
